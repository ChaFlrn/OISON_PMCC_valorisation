...
observateurs_tableau <- observateurs %>%
  ...
  mutate(total = rowSums(select(., -annee), na.rm = TRUE)) %>% # calcul les totaux
  mutate(annee = as.character(annee)) %>% # met annee en caractere
  filter(!is.na(INSEE_DEP)) # Supprime les lignes sans la mention du département
...
<br>
<br>
Dans ce graphique double : 
- à gauche: tous les départements
- à droite :les 5 départements (`r top_dep`) ayant le plus d'observateurs en `r params$annee_fin`.

```{r , fig.width = 6.5 , fig.height = 3.5}
# Graphique général
p_all <- ggplot(observateurs) +
...
```{r , fig.width = 6 , fig.height = 3.5}
cols_numeric <- which(sapply(groupes_espece_tableau, is.numeric))
...
for(i in 1:12){
  create_beautiful_radarchart(
    ...
    caxislabels = seq.int(0,100,length.out = 5),  
...
cols_numeric <- which(sapply(groupes_espece_pro_tableau, is.numeric))
...
ggplot(groupes_espece_pro)+
  ...
  geom_text(data=tail(groupes_espece_pro,2), #CHANGER LE CHIFFRE POUR AFFICHER =/- de NOMS
            aes(x = 1.8, 
                ...
                size = 3)+
  labs(
    title = paste("Nombre régional de saisies par groupe d'espèces protégées en", params$annee_fin, nom_bases),
    fill = "Groupes d'espèces"
    ) +
  theme(
    text = element_text(size = 9),
