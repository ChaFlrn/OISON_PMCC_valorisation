...
- normale : toutes les autres espèces non concernées par le statut "menacée" ou "envahissante"
...
Présentation du nombre de saisies par statut en fonction des différents départements pour l'année `r params$annee_fin`.
...
L'évolution des saisies d'espèces protégées, au niveau régional, en fonction des différents groupes d'espèces, de `r params$annee_debut` à `r params$annee_fin`.
...
ggplot(observations_espece_pro) +
  ...
  theme(text = element_text(size = 9),
      plot.title = element_text(size = 9) )
```
Cette carte de chaleur représente les saisies par des ronds, la couleur est fonction du statut de la réglementation de l'espèce. L'intensité de la couleur gradue le nombre de saisies sur le secteur concerné. \newpage
