...
L'évolution du nombre régional de saisies par groupes d'espèces est représentée par les deux graphiques ci-dessous :
...
ggplot(groupes_espece_par_annee) +
  ...
  geom_smooth(aes(colour = GROUP2_INPN), se = FALSE) +
  geom_point()+
...
\newpage
Le détail du nombre de saisies pour l'année `r params$annee_fin`, à l'échelle régionale, par groupe d'espèces, est présenté dans le tableau ci-dessous.
```{r}
...
flextable(groupes_espece_tableau_region) %>%
  ...
  autofit() # ajuste la largeur des colonnes automatiquement, si ne convient pas utiliser "width(width = 0.5)"
```
...
Cette évolution est également représentée par les graphiques ci-dessous :


- évolution du nombre régional de saisies au fil des années


- nombre de saisies par département et par statut pour l'année `r params$annee_fin`
...
L'évolution des saisies d'espèces protégées, à l'échelle régionale, est présentée dans le tableau ci-dessous, données de `r params$annee_debut` à `r params$annee_fin`.
```{r}
...
L'évolution du nombre de saisies régionales des espèces protégées est représentée par les graphiques ci-dessous :


- évolution du nombre de saisies au fil des années


- nombre de saisies par statut réglementaire et par département pour l'année `r params$annee_fin`.
...
ggplot(espece_pro_dep) +
  ...
  labs(
    ...
    y = "Nombre de saisies",
    fill = "Réglementation",
    title = paste("Nombre de saisies par statut réglementaire et par département en", params$annee_fin, nom_bases)) +
  theme_minimal() +
  theme(text = element_text(size = 9))
```
