...
Evolution du nombre de saisies des données `r nom_bases`, de `r params$annee_debut` à `r params$annee_fin`, pour le SD `r params$dep`.
```{r}
...
Cette évolution du nombre de saisies est représentée par le grahique ci-dessous.
...
Evolution du nombre d'observateurs `r nom_bases` dans le SD `r params$dep`, de `r params$annee_debut` à `r params$annee_fin`. 
```{r}
...
Cette évolution du nombre d'observateurs est représentée par le graphique ci-dessous.
...
## Evolution du nombre de saisies par observateur
Evolution du nombre moyen de saisies `r nom_bases`, par observateurs pour le SD `r params$dep` en fonction de la moyenne régionale, de `r params$annee_debut` à `r params$annee_fin`.

```{r}
saisies_observateur_dep <- saisies_dep %>%
  left_join(observateurs_dep) %>%
  mutate(moyenne_saisies_observateur_dep =
           ifelse(!is.na(nombre_observateurs_dep)
                  & nombre_observateurs_dep > 0, 
                  round(nombre_saisies_dep / nombre_observateurs_dep,1), 
                  0)) %>% 
...
flextable(saisies_observateur_tableau) %>%
  ...
  autofit()
```
Cette évolution du nombre moyen de saisies par observateurs est représentée par le graphique ci-dessous.
...
Le graphique suivant illustre la répartition du nombre de saisies par observateur. Pour un nombre de saisie donné, plus la zone colorée est large, plus il y a d'observateurs concernés. Le carré noir symbolise la médiane alors que le losange jaune identifie la moyenne.
...
ggplot(saisies_observateur_violon, aes(x = factor(INSEE_DEP), y = nombre_saisies, fill=INSEE_DEP)) +
  ...
  labs(
    x = paste("SD",params$dep),
    y = "Nombre de saisies par observateur",
    title = paste("Répartition du nombre de saisies par observateur \npour le SD", params$dep,"en",params$annee_fin, nom_bases)) +
  theme_minimal()+
...
L'évolution du nombre de saisies par groupe d'espèces est représentée par les graphiques ci-dessous :


- évolution du nombre des saisies au fil des années


- répartition de ces saisies en fonction des groupes d'espèces pour l'année `r params$annee_fin`
...
ggplot(groupes_espece)+
  ...
  labs(
    ...
    title = paste("Répartition des saisies par groupe d'espèces pour le SD", params$dep,"en",params$annee_fin,nom_bases)
    ) +
  theme(
...
Comparaison du nombre de saisies faites par le SD `r params$dep` par groupe d'espèces, pour l'année `r params$annee_fin`, avec le total régional. La colonne "%" représente la proportion à l'échelle départementale de ce groupe d'espèce. La colonne "contribution" est la part des saisies départementales à l'échelle régionale. 
...
```
L'évolution du nombre de saisies par groupe d'espèces, en excluant le groupe Oiseaux pour une meileure visibilité, est représentée par les deux graphiques ci-dessous :

- évolution du nombre des saisies au fil des années


- répartition de ces saisies en fonction des groupes d'espèces, de `r params$annee_debut` à `r params$annee_fin`


- focus sur l'année `r params$annee_fin`
```{r}
if (params$OISON) {
...
# Create radar charts
  create_beautiful_radarchart(
    ...
    title = paste("SD",radar_especes_dep$INSEE_DEP[3])   # Ensure titles is defined
  )
par(op)
```
```{r}
...
if (params$OISON) {
  ggplot(groupes_espece_sans_oiseaux) +
    ...
    geom_text(data = tail(groupes_espece_sans_oiseaux, 3),
              aes(x = 1.8, 
                  y = cumulative, 
                  label = paste(GROUP2_INPN, "\n", pourcentage, "%")),
                  size = 3) +
...
L'évolution du nombre de saisies par statut pour le SD `r params$dep` est décrite dans le tableau ci-dessous, pour les années `r params$annee_debut` à `r params$annee_fin`. 
...
Cette évolution du nombre de saisies par statut est illustrée par le graphique ci-dessous. 
...
ggplot(statuts_espece_par_annee) +
  ...
  geom_smooth(se = FALSE) +
  geom_point()+
  geom_text(aes(label = nb_saisies, x = annee),
            position = position_nudge(y = max(statuts_espece_par_annee$nb_saisies)/15),
            size = 3,
            show.legend = FALSE) +
...
L'évolution du nombre de saisies `r nom_bases` des espèces protégées pour le SD `r params$dep` est dressée dans le tableau ci dessous, de `r params$annee_debut` à `r params$annee_fin`.
...
L'évolution du nombre de saisies d'espèces protégées `r nom_bases`, en fonction des différents groupes, est décrite dans le tableau ci-dessous. Ce tableau décrit également la contribution départementale pour la région pour chaque groupe d'espèces pour l'année `r params$annee_fin`.
...
Cette évolution du nombre de saisies des espèces protégées est représentée par les deux graphiques ci-dessous :
...
ggplot(espece_pro_par_annee) +
  ...
  geom_point()+
  geom_text(aes(label = nb_saisies, x = annee),
            position = position_nudge(y = max(espece_pro_par_annee$nb_saisies)/15),
            size = 3,
            show.legend = FALSE) +
...
ggplot(groupes_espece_pro)+
  ...
  geom_text(data=tail(groupes_espece_pro,3), #CHANGER LE CHIFFRE POUR AFFICHER PLUS/MOINS DE NOMS
            aes(x = 1.8, 
                y = cumulative, 
                label=paste(GROUP2_INPN,"\n",pourcentage, "%")),
                size = 3)+
