...
ggplot(saisies_observateur_violon, aes(x = factor(INSEE_DEP), y = nombre_saisies, fill=INSEE_DEP)) +
  ...
  stat_summary(fun = mean, geom = "point", shape = 18, size = 3, color = "yellow", fill = "yellow") +
  ...
  labs(
    ...
    - Le losange jaune représente la moyenne"
  ) +
  theme_minimal()+
...
ggplot(statuts_espece_dep) +
  aes(x = statut_LR, fill = statut_LR) +
...
ggplot(espece_pro_dep) +
  aes(x = espece_pro, fill = espece_pro) +
  geom_bar(position = "dodge2") +
  scale_fill_manual(values = colors_pro) +
  labs(
    x = "Réglementation",
    y = "Nombre de saisies",
    fill = "Départements",
    title = paste("Nombre de saisies des espèces protégées \npour le SD", params$dep,"en", params$annee_fin, nom_bases)) +
  theme_minimal() +
  theme(text = element_text(size = 9),
        legend.margin = margin(t = 0, r = 200, b = 0, l = 0),
        legend.position = "none")
